...
class Plugin(object):
    def poll(self):
        if self.has_poll:
            ...
            self._poll()
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                logging.getLogger().debug("Plugin [{}] poll on-thread [{}] ms".format(self.name, str(int((time.time() - time_start) * 1000))))
    ...
    def push(self, text_content):
        if self.has_push:
            ...
            if logging.getLogger().isEnabledFor(logging.DEBUG):
                logging.getLogger().debug("Plugin [{}] push on-thread [{}] ms".format(self.name, str(int((time.time() - start_time) * 1000))))
    ...
    def datum_push(self, data_metric, data_temporal, data_type, data_value, data_unit, data_scale, data_timestamp, bin_timestamp, bin_width,
        ...
        if data_value is not None:
            ...
            if DATUM_QUEUE_LAST not in datums_deref or datums_deref[DATUM_QUEUE_LAST]["data_value"] != datum_dict["data_value"] or \
                ...
                if logging.getLogger().isEnabledFor(logging.DEBUG):
                    logging.getLogger().debug(
                        "Plugin [{}] dropped datum [{}]".format(self.name, self.datum_tostring(datum_dict)))
