...
class Plugin(object):
        ...
        if "push" in self.config and self.config["push"]:
            for datum_metric in self.datums:
                for datum_type in self.datums[datum_metric]:
                    for datum_bins in self.datums[datum_metric][datum_type]:
                        # noinspection PyCompatibility
                        for i in xrange(len(self.datums[datum_metric][datum_type][datum_bins]["queue"])):
                            datum_avro = self.datums[datum_metric][datum_type][datum_bins]["queue"].popleft()
                            if logging.getLogger().isEnabledFor(logging.DEBUG):
                                logging.getLogger().debug("Popped datum [{}]".format(self.datum_tostring(self.datum_deserialise(datum_avro))))
    ...
    def datum_push(self, data_metric, data_type, data_value, data_unit, data_scale, data_timestamp, bin_timestamp, bin_width, bin_unit):
        ...
        try:
            datum_avro = self.datum_serialise(datum_dict)
    ...
    def datum_tostring(self, datum_dict):
        return "{}.{}={}{}".format(
            ...
            int(self.datum_deserialise(self.datum_serialise(datum_dict))["data_value"]) / Decimal(datum_dict["data_scale"]),
            datum_dict["data_unit"]
        )

    @staticmethod
    def datum_serialise(datum_dict):
    ...
    @staticmethod
    def datum_deserialise(datum_avro):
